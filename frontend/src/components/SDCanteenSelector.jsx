...
const SDCanteenSelector = () => {
  const { selectedCanteenId, selectCanteen, canteens, setCanteens } =
    useContext(SDCanteenContext);
      ...
      try {
        ...
        if (isStaff && user?.assignedCanteens) {
          allCanteens = allCanteens.filter((canteen) =>
            user.assignedCanteens.some(
              (assignedId) => assignedId.toString() === canteen._id.toString(),
            ),
          );
        }
