    ...
    if (!selectedCanteenId) {
        return (
            <div className="bg-white rounded-2xl shadow p-6 text-center">
                <p className="text-gray-500">Please select a canteen to view inventory</p>
            </div>
        );
    }
                ...
                <table className="w-full text-sm">
                    <thead>
                        <tr className="bg-gray-50 border-b">
                            <th className="px-5 py-3 text-left font-medium text-gray-600">Item</th>
                            <th className="px-4 py-3 text-left font-medium text-gray-600">Category</th>
                            <th className="px-4 py-3 text-center font-medium text-gray-600">Price</th>
                        ...
                        {items.map(item => {
                            ...
                            return (
                                <tr key={item._id} className={`hover:bg-gray-50 transition ${isLowStock ? 'bg-red-50' : ''}`}>
                                    ...
                                    <td className="px-4 py-3 text-center font-medium text-gray-500">Rs. {item.price}</td>
                                    <td className="px-4 py-3 text-center">
                                        <div className="flex items-center justify-center gap-2">
                                            ...
                                            <span className={`font-bold text-base w-10 text-center ${isLowStock ? 'text-red-600' : 'text-gray-900'}`}>
                                                {item.currentStock}
                                            </span>
                                            <button
                                                onClick={() => updateStock(item._id, 1)}
