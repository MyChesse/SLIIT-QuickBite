...
const AddAnohanaCanteenPromotion = () => {
  ...
  const content = (
    <section className="rounded-[2rem] border border-slate-200 bg-white p-8 shadow-[0_30px_70px_-30px_rgba(15,23,42,0.08)]">
      <div className="mb-8">
        <p className="text-xs uppercase tracking-[0.35em] text-primary font-semibold">Mini Canteen</p>
        <h1 className="mt-3 text-4xl font-extrabold text-slate-950">Add Promotion</h1>
        <p className="mt-3 max-w-2xl text-sm text-slate-500">Add a fresh seasonal promotion for the Mini Canteen with a polished admin experience.</p>
      </div>
      <PromotionForm canteenName="Mini Canteen" onSubmit={handleSubmit} />
    </section>
  );
