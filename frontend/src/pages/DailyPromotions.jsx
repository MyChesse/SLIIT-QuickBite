...
const DailyPromotions = () => {
  ...
  const addPromotionToCart = (promotion) => {
    if (!promotion) {
      toast.error("Promotion item not found");
      return;
    }
    ...
    const promotionPrice = Number(promotion.discountedPrice);
    ...
    const item = {
      _id:
        promotion._id ||
        `promo-${String(promotion.canteenName || "canteen")
          .replace(/\s+/g, "-")
          .toLowerCase()}-${String(promotion.title || "item")
          .replace(/\s+/g, "-")
          .toLowerCase()}`,
      name: promotion.title || "Promotion Item",
  ...
  const renderEmptyState = (icon, title) => (
    <div className="flex flex-col items-center justify-center rounded-3xl border-2 border-dashed border-slate-300 bg-white/70 px-8 py-16 text-center">
      ...
      <h3 className="mb-2 text-xl font-bold text-slate-600">
        No promotions available today
      </h3>
      <p className="text-sm text-slate-500">
        {title} promotions will appear here once published.
      </p>
    </div>
  );
    ...
    return (
      <section key={section.key} className="mb-16">
        ...
        {sectionPromotions.length === 0 &&
          renderEmptyState(section.emptyIcon, section.title)}

        {sectionPromotions.length > 0 && (
          <div className="grid grid-cols-1 items-stretch gap-5 xl:grid-cols-12">
            <div
              className={`rounded-2xl border border-[#dbe2ef] bg-[#edf2fb] p-3 shadow-sm xl:col-span-8 ${section.reverse ? "xl:order-2" : ""}`}
            >
              <article className="group relative grid h-full min-h-[390px] grid-cols-1 overflow-hidden rounded-xl bg-white transition-all duration-500 hover:-translate-y-1 hover:shadow-[0_22px_42px_-24px_rgba(15,79,185,0.75)] xl:grid-cols-12">
                <div className="relative h-[300px] overflow-hidden xl:col-span-7 xl:h-full">
                  ...
                  <div className="absolute left-3 top-3 flex items-center gap-2">
                    <span className="rounded-full bg-[#0f4fb9] px-2.5 py-1 text-[10px] font-bold uppercase tracking-[0.14em] text-white">
                      {section.badge}
                    </span>
                    <span className="rounded-full bg-white/85 px-2.5 py-1 text-[10px] font-bold uppercase tracking-[0.14em] text-slate-700">
                      Available
                    </span>
                  </div>
                </div>
                ...
                <div className="flex flex-col justify-between p-5 xl:col-span-5 xl:p-6">
                  <div>
                    <p className="text-[11px] font-bold uppercase tracking-[0.18em] text-[#c26b12]">
                      Daily Special
                    </p>
                    <h3 className="mt-2 text-xl font-bold leading-tight text-slate-900 transition-colors duration-300 group-hover:text-[#0f4fb9] xl:text-2xl">
                      {featured?.title || "Untitled promotion"}
                    </h3>
                  ...
                  <div className="mt-5 space-y-4">
                    <div className="flex items-end justify-between">
                      <div>
                        ...
                        <p className="text-3xl font-extrabold leading-none text-[#0f4fb9] transition-transform duration-300 group-hover:translate-x-1 xl:text-4xl">
                          Rs. {featured?.discountedPrice ?? "0"}
                        </p>
                      </div>
                      <div className="text-right">
                        <p className="text-xs text-slate-500">Was</p>
                        <p className="text-sm text-slate-400 line-through">
                          Rs. {featured?.originalPrice ?? "0"}
                        </p>
                        {featured && (
                    ...
                    <button
                      onClick={() => addPromotionToCart(featured)}
                      className="group/cta w-full rounded-md bg-[#0f4fb9] px-4 py-2.5 text-xs font-bold uppercase tracking-[0.08em] text-white transition-all duration-300 hover:bg-[#0c4299] hover:shadow-[0_10px_20px_-12px_rgba(15,79,185,0.9)]"
                    >
                ...
                <div className="pointer-events-none absolute right-4 top-4 hidden w-56 rounded-xl border border-[#d6dff0] bg-white/95 p-3 shadow-[0_16px_30px_-18px_rgba(15,79,185,0.7)] opacity-0 backdrop-blur-sm transition-all duration-300 group-hover:translate-y-0 group-hover:opacity-100 xl:block -translate-y-2">
                  <p className="text-[10px] font-bold uppercase tracking-[0.18em] text-[#0f4fb9]">
                    Quick View
                  </p>
                  ...
                  <p className="mt-1 text-xs text-slate-600">
                    Limited-time campus deal
                  </p>
                  <div className="mt-2 flex items-center justify-between">
                    <span className="text-sm font-extrabold text-[#0f4fb9]">
                      Rs. {featured?.discountedPrice ?? "0"}
                    </span>
            ...
            <div
              className={`space-y-4 xl:col-span-4 ${section.reverse ? "xl:order-1" : ""}`}
            >
              {secondary.map((promotion) => (
                <article
                  key={promotion._id}
                  className="group relative rounded-xl border border-[#dde4ef] bg-white p-3 shadow-sm transition-all duration-300 hover:-translate-y-1 hover:shadow-[0_18px_30px_-20px_rgba(15,79,185,0.75)]"
                >
                  <div>
                    ...
                    <div className="min-w-0 pt-3">
                      <div className="mb-1 flex items-start justify-between gap-2">
                        <p className="line-clamp-2 text-sm font-bold leading-tight text-slate-900 transition-colors duration-300 group-hover:text-[#0f4fb9]">
                          {promotion.title}
                        </p>
                        <span className="rounded bg-[#fed7aa] px-1.5 py-0.5 text-[9px] font-bold uppercase text-[#9a3412]">
                          Offer
                        </span>
                      </div>
                      <p className="mt-2 text-base font-bold text-[#0f4fb9]">
                      ...
                      <button
                        ...
                        className="mt-3 inline-flex w-full items-center justify-center rounded-md bg-[#0f4fb9] px-3 py-2 text-[11px] font-bold uppercase tracking-[0.08em] text-white transition-all duration-300 hover:bg-[#0c4299]"
                      >
                        Add to Cart
                      </button>
                    </div>
                  ...
                  <div className="pointer-events-none absolute left-3 top-3 rounded-lg border border-[#d6dff0] bg-white/95 px-2.5 py-1.5 shadow-md opacity-0 transition-all duration-300 group-hover:translate-y-0 group-hover:opacity-100 -translate-y-1">
                    <p className="text-[9px] font-bold uppercase tracking-[0.14em] text-[#0f4fb9]">
                      Preview
                    </p>
    ...
    <main className="min-h-screen w-full bg-[#eff2f8]">
      ...
      <div className="mx-auto w-full max-w-7xl px-3 pb-20 pt-28 md:px-6 lg:px-8">
        <header className="mb-10 pb-6 text-center md:text-left">
          <span className="mb-1 block text-[11px] font-bold uppercase tracking-[0.28em] text-[#1d4fa5]">
            Exclusive Deals
          </span>
          <h1 className="text-4xl font-extrabold tracking-tight text-[#0f4fb9] sm:text-5xl">
            Daily Promotions
          </h1>
          <p className="mx-auto mt-3 max-w-3xl text-sm leading-relaxed text-slate-600 sm:text-base md:mx-0">
            Discover fresh promotions at all SLIIT canteens. Hand-picked,
            time-sensitive, and available for today only.
          </p>
        </header>
        ...
        {!loading &&
          !error &&
          canteenSections.map((section) => renderPromotionSection(section))}
      </div>
    </main>
  );
};
